...
const { compareLastMonth, sumSales, kFormatter } = require('../helper/compareLastMonth');
...
class OrderController {
  createOrder = async (req, res) => {
    try {
      ...
      console.log(
        '🚀 ~ file: OrderController.js:17 ~ OrderController ~ createOrder= ~ req.body',
        req.body
      );
  ...
  readOrderDetail = async (req, res) => {
    try {
      ...
      const Products = await Product.find();
      arrayFilterProduct = Products.filter((item) => arrayID.includes(item._id.toString()));
        ...
        arrayFilterProduct.forEach((item) => {
          if (item._id.toString() === arrayID[i]) {
            const newProduct = {
              ...others[i],
              productId: arrayID[i],
              img: item._doc.photo?.url ?? item._doc.img,
  ...
  readIncome = async (req, res) => {
    ...
    arrayDate[0] = arrayDate[0].length === 2 ? arrayDate[0] : '0' + arrayDate[0];
    arrayDate[1] = arrayDate[1].length === 2 ? arrayDate[1] : '0' + arrayDate[1];
    ...
    //Find seven days ago
    const dateNow7 = new Date(`${arrayDate[2]}-${arrayDate[0]}-${arrayDate[1]}`);
    ...
    const sevenDaysAgoLastYear = new Date(dateNow7LastYear.setDate(dateNow7LastYear.getDate() - 6));
    dateNow7LastYear.setDate(dateNow7LastYear.getDate() + 6);

    //Find 30 days ago
    const dateNow30 = new Date(`${arrayDate[2]}-${arrayDate[0]}-${arrayDate[1]}`);
  ...
  readCardInformation = async (req, res) => {
    ...
    try {
      ...
      const compareUser = compareLastMonth(usersCurrentMonth.length, usersLastMonth.length);
      const compareOrder = compareLastMonth(ordersCurrentMonth.length, ordersLastMonth.length);
  ...
  generatePDF = (req, res) => {
    ...
    pdf.create(pdfTemplate(data), options).toFile(`${__dirname}/result.pdf`, (err) => {
      if (err) {
